...
Emphasis here is that we are just looking for relative performance rather than absolute performance of a predictor pair. 

In this example, the native range for the species is South Africa
```{r}
e <- extent (70, 135, 0, 40)
...
dist <- insect_dist[insect_dist$Species =="b_dorsalis" & insect_dist$Range == "Native",]
...
model.out <- che_model(spp = "b_dorsalis", parallel = TRUE, model.data=model.data,
                       n.cores = 2)
